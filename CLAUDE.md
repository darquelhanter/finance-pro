...
**The only server-side routes are the Gemini-backed ones**, because `GEMINI_API_KEY` must never reach the client bundle: `POST /api/ia/extrair-fatura` (parses a fatura PDF/image/text via `GeminiService`), `POST /api/ia/insights` (generates financial insights from a client-computed `DashboardResumo`), and `GET /api/schema-sql` (serves `src/services/schema_financepro.sql` for the in-app "view schema" modal). All three require a Firebase ID token (`Authorization: Bearer <token>`, obtained client-side via `fetchAutenticado()`/`obterTokenAtual()` in `auth.service.ts`) verified in `api/_lib/auth.ts` — this exists to stop the deployed URL being used as a free, unauthenticated proxy to the paid Gemini API. `api/_lib/auth.ts` verifies the token directly against Google's public JWKS via `jose` (`createRemoteJWKSet` + `jwtVerify`) rather than using `firebase-admin` — the Admin SDK's `jwks-rsa`/`jose` dependency chain crashes with `ERR_REQUIRE_ESM` on Vercel's Node runtime, and plain JWKS verification needs no service-account credentials at all.

**Every relative import reachable from `api/*.ts` needs an explicit `.js` extension** (e.g. `from './_lib/handlers.js'`), even though the file on disk is `.ts` and `tsconfig.json` uses `moduleResolution: "bundler"`. Vercel runs each function's compiled output through Node's native ESM loader (`package.json` has `"type": "module"`), which — unlike Vite/esbuild/tsx — refuses to resolve an extensionless relative specifier (`ERR_MODULE_NOT_FOUND`). This only breaks in production; local `npm run dev` (tsx) and `vite build` both resolve extensionless imports fine, so this class of bug is invisible until you check a real Vercel deploy.
